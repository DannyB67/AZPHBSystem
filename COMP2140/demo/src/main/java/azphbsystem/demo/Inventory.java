...
public class UserDateInput {
    ...
    private static String SendRequest() throws Exception{
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(new URI("http://localhost/AZPHBSystem/database.php?request=events")).GET().build();
    ...
    private static void displayItems(String ite) {
        ...
        System.out.println("----------------------------------------");
           //System.out.println("Item Name + | Item Count | Item Description");
        String[] item = ite.replace("]","").replace("[","").replace("{","").replace("\"","").replace(",","").split("}");
        for (String thing : item) {
            System.out.println("\n");
    ...
    public static void main(String[] args) {
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run(){
                try {
                    displayItems(SendRequest());
                } catch (Exception e) {
                    System.err.println("You have a problem "+e);
                }
            }
        });
